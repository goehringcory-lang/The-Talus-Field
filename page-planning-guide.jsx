...
function PlanningGuide({ go }) {
  ...
  const gettingThere = find("getting-to-yosemite");
  const wildernessPermits = find("yosemite-wilderness-permits-guide");
  ...
  const accessibility = find("yosemite-accessibility-guide");
  const pets = find("pets-in-yosemite");
  const rangerPrograms = find("yosemite-ranger-programs");
  ...
  return (
    <div className="page">
      ...
      <div className="wrap" style={{ paddingTop: 56 }}>
        ...
        {/* Part Two: Getting there and getting in */}
        <section style={{ paddingTop: 32, paddingBottom: 56, borderTop: "1px solid var(--rule)" }}>
          <div className="eyebrow eyebrow--moss" style={{ marginTop: 32, marginBottom: 12 }}>Part Two</div>
          <h2 style={sectionH2}>Getting there and getting in</h2>
          <p style={sectionLede}>
            Five entrances, four highways, one seasonal pass that does not exist half the year, and a permit system guarding the 95 percent of the park most visitors never see. The logistics of arrival, and the paperwork for going deeper.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 36, rowGap: 48 }}>
            <ArticleCard article={gettingThere} go={go} />
            <ArticleCard article={wildernessPermits} go={go} />
          </div>
        </section>

        {/* Part Three: When you arrive */}
        <section style={{ paddingTop: 32, paddingBottom: 56, borderTop: "1px solid var(--rule)" }}>
          ...
          <p style={sectionLede}>
            What is in the car, who you are traveling with, whether everyone in your group can hike. The pragmatic decisions that make a Yosemite day flow or stall. The cooler, the camp chair, the Junior Ranger booklet, the bridge view from a wheelchair, the dog.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 36, rowGap: 48 }}>
            ...
            <ArticleCard article={accessibility} go={go} />
            <ArticleCard article={pets} go={go} />
            <ArticleCard article={rangerPrograms} go={go} />
          </div>
        </section>

        {/* Part Four: If you're hiking Half Dome */}
        <section style={{ paddingTop: 32, paddingBottom: 56, borderTop: "1px solid var(--rule)" }}>
        ...
        {/* Part Five: The seasonal calendar */}
        <section style={{ paddingTop: 32, paddingBottom: 56, borderTop: "1px solid var(--rule)" }}>
          <div className="eyebrow eyebrow--moss" style={{ marginTop: 32, marginBottom: 12 }}>Part Five</div>
