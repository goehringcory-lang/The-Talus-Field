...
- **API** https://api.thetalusfieldjournal.com — Worker `talus-field-guide-api` (`workers/`). Deploys on merge to `main` through a gated Workers Build: the typecheck and tests run first, and a failure ships nothing and leaves the previous version live. A live value that disagrees with `workers/wrangler.toml` means the deploy is stale (a failed build or a rollback), never that the repo is wrong.
...
5. **Stale deploy**: it cannot be redeployed from here. Report it as the top-line item with the Worker version id and timestamp from the parity check or the log, note that any merge to `main` (including this sweep's own PR) re-triggers the Workers Build and clears the symptom, and point the owner at the Workers Build deploy history in the Cloudflare dashboard for what rolled the version back.
...
1. **Editorial staleness probe**: `cd scripts && node system-checks.mjs --online --only=deploy-parity` (`npm run parity:check`). It reads the live Worker's own catalog off `/.well-known/talus-build.json`, the live `/articles.json`, and the repo, and errors on the known signature (the asset layer serving articles the Worker bundle lacks), naming the slugs and the Worker version id. Then read the last seven records of `scripts/data/deploy-parity-log.json` (written nightly): a verdict that flipped, or a version id that changed on a night with no merge, is what the owner needs to look up in the Cloudflare deploy history. A new version id on a night with only the nightly chore commit is expected, since that commit re-triggers the Workers Build. If the route answers 404 (the check says "not deployed yet"), fall back to fetching the three newest slugs in `window.ARTICLES` (`data.js`, by date) at `https://thetalusfieldjournal.com/articles/<slug>`; any 404 is the signature.
...
2. `npm --prefix scripts run checks:online` — the full battery (links, per-page SEO, og:image, JSON-LD, sitemap, feed, image hygiene, mirror freshness, template smoke, SPF/DKIM/DMARC, API probes). **Errors** are sale- or integrity-breaking by design: fix per policy or report top-line. **Warnings** are judgment calls: fix the cheap ones, report the rest. Read findings against `scripts/data/baseline-report.md`; new ones matter more than standing ones. The environment has full Internet access (`ROUTINES.md`, "Network access"), so a failed probe is a finding: an error or timeout from the host itself goes in the report as what it is. The one exception is a CONNECT 403 from the agent proxy on every live host (`curl -sS "$HTTPS_PROXY/__agentproxy/status"` shows the refusals): that is the environment's network policy regressing, not the site. Run the offline battery instead, say in the report that deploy parity and the API gate were not verified this week, and put the regression under "Needs your decision". Never report a proxy 403 as an outage.
