...
import { formatNight, formatNights } from './availabilityMatch'
...
// Campsite opening for a watch the buyer set on the Openings tab (the
// five-minute sweep in lib/availabilitySweep.ts). Sent at whatever hour the
// site appears: a cancellation lands at any hour and is gone in minutes, so
// a morning digest would be a list of sites somebody else already has. The
// button goes straight to the campground page on recreation.gov; the watch
// link is the off switch.
export async function sendAvailabilityAlert(
  ...
  args: {
    ...
    openings: Array<{
      date: string
      sites: Array<{ id: string; site: string; loop: string }>
      qty: number | null
    }>
    bookUrl: string
    watchUrl: string
  ...
  const { to, campground, mode, dates, openings, bookUrl, watchUrl } = args
  const label = formatNights(dates)
  const subject = `${campground}: a site opened for ${label}`
  const lead =
    mode === 'full'
      ? `One site at ${campground} is open for every night you asked for, ${label}.`
      : `${campground} has an opening on your nights: ${label}.`

  // One line per night. Site numbers are what the reader types into the
  // recreation.gov filter; Camp 4 sells spots, not sites. A loop letter that
  // the site number already starts with ("C" / "C064") is not repeated.
  const siteLabel = (s: { site: string; loop: string }) =>
    s.site.startsWith(s.loop) ? s.site : `${s.loop} ${s.site}`.trim()
  const lines = openings.map((o) => {
    const day = formatNight(o.date)
    if (o.qty !== null) return `${day}: ${o.qty} ${o.qty === 1 ? 'spot' : 'spots'}`
    const names = o.sites.slice(0, 5).map(siteLabel)
    const more = o.sites.length > 5 ? ` and ${o.sites.length - 5} more` : ''
    return `${day}: ${o.sites.length} ${o.sites.length === 1 ? 'site' : 'sites'} (${names.join(', ')}${more})`
  })
  ...
  const text = [
    ...
    watchUrl,
    ``,
    `Sites go in minutes. Alerts arrive at whatever hour a site appears; delete the watch in the guide to stop them.`,
    `— Cory`,
  ].join('\n')
    ...
    <div style="background:#f1ead6;padding:36px 16px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;margin:0 auto;">
        <tr>
          <td style="padding:0 0 18px;">
            <div style="font-family:${serif};font-size:24px;color:#14110c;">The Talus Field</div>
            <div style="font-family:${sans};font-size:10px;font-weight:700;letter-spacing:3px;text-transform:uppercase;color:#50402e;padding-top:4px;">The Field Guide &middot; Campsite opening</div>
          </td>
        </tr>
        <tr>
          <td style="border-top:1px solid #2a2118;padding:24px 0 0;">
            <p style="font-family:${serif};font-size:17px;line-height:1.55;color:#14110c;margin:0 0 18px;">${escapeHtml(lead)}</p>
            <ul style="font-family:${sans};font-size:14px;line-height:1.6;color:#14110c;margin:0 0 22px;padding-left:18px;">
              ${lines.map((line) => `<li>${escapeHtml(line)}</li>`).join('')}
            </ul>
            <p style="margin:0 0 26px;">
              <a href="${escapeHtml(bookUrl)}" style="display:inline-block;padding:14px 22px;background:#14110c;color:#f1ead6;text-decoration:none;font-family:${sans};font-weight:600;letter-spacing:2px;text-transform:uppercase;font-size:13px;">
                Book on recreation.gov
              </a>
            </p>
            <p style="font-family:${sans};font-size:13px;color:#50402e;margin:0 0 6px;">Sites go in minutes. Alerts arrive at whatever hour a site appears; <a href="${escapeHtml(watchUrl)}" style="color:#50402e;">delete the watch in the guide</a> to stop them.</p>
            <p style="font-family:${sans};font-size:13px;color:#50402e;margin:0;">&mdash; Cory</p>
          </td>
        </tr>
      </table>
