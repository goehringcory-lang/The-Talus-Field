...
- **Routes** (see `App.tsx`): `/open` (magic-link landing), `/login`, `/` (region picker), `/region/:regionId`, `/stop/:stopId`, `/essentials` + `/essentials/:topicId` (know-before-you-go, incl. the `before-you-go` night-before checklist and the packing checklist), `/secret-spots`, `/search`, `/programs` (trip-dates → date-grouped program list), `/trip` (day-by-day planner; the review step's calendar sheet offers a hosted subscription feed or a one-time ICS), `/map` (MapLibre GL over the Worker's `/tiles` raster topo proxy — NOT Google Maps; pin styles and the Google Maps directions deeplink live in `src/map/kinds.ts`), `/account` (DownloadManager). Everything except `/open` and `/login` is wrapped in `RequireAuth`. `/trip/*` subpaths redirect to `/trip`.
