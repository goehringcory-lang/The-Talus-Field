...
- **PWA localStorage keys** beyond `tfg.jwt`: `tfg.downloads` (pack completion), `tfg.favorites`, `tfg.install.dismissed`, `tfg.beforeYouGo.dismissed`, `tfg.trip.dates` (shared by `/programs` and `/trip`), `tfg.trip.plan` (the planner, zod-parsed drop-on-corrupt), `tfg.trip.feed` (hosted calendar-feed record: token + URL + last-publish stamp), `tfg.programs.meta` (programs cache stamp). The programs payload itself lives in the Cache API bucket `tfg-data` (unversioned, survives deploys like `tfg-tiles`) — see `src/programs/cache.ts`.
