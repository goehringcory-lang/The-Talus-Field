...
- **Coords were audited (2026-07) but some still need a field pass.** Stop coords in `stops.ts` carry either `// verified: <source> 2026-07` (multiple authoritative sources agree) or `// TODO: verify` (unsigned pullouts, ambiguous lots — only a field visit confirms). Never strip a TODO marker without ground truth, and never invent a coordinate as final. `content/landmarks.ts` is the separate major-destinations reference layer (~45 records, zod-validated with a Yosemite-bbox check that catches swapped lat/lng); it renders as hollow toggle-able pins on `/map` and as the `/essentials/gps-directory` reference page (a custom route that outranks `/essentials/:topicId`; its `essentials.ts` entry is a stub feeding the index and search).
- **Routes** (see `App.tsx`): `/open` (magic-link landing), `/login`, `/` (region picker), `/region/:regionId`, `/stop/:stopId`, `/essentials` + `/essentials/:topicId` (know-before-you-go, incl. the `before-you-go` night-before checklist and the packing checklist), `/essentials/gps-directory` (landmarks coordinate reference, custom page), `/secret-spots`, `/search`, `/programs` (trip-dates → date-grouped program list), `/trip` (day-by-day planner + ICS export), `/map` (MapLibre GL over the Worker's `/tiles` raster topo proxy — NOT Google Maps; pin styles and the Google Maps directions deeplink live in `src/map/kinds.ts`), `/account` (DownloadManager). Everything except `/open` and `/login` is wrapped in `RequireAuth`. `/trip/*` subpaths redirect to `/trip`.
