...
// bulletin.json is fetched from two pages, each with its own const. They
// must all carry the same ?v=: the file is served with a long TTL, so a page
// reading it under a stale number renders the previous edition beside the other page
...
if (STAMP) {
  ...
  const out = {
    __comment:
      ...
      "(index.html shared number, BODY_VERSIONS, POINTS_URL, BULLETIN_URL/TIOGA_BULLETIN_URL, " +
...
if (problems.length) {
  ...
  console.error(
    `\nFix, in order:\n` +
      ...
      `     its BODY_VERSIONS entry, POINTS_URL, BULLETIN_URL + TIOGA_BULLETIN_URL together,\n` +
      `     or the /img/ counter).\n` +
