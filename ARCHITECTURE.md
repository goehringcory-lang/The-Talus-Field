...
| `NAV_GROUPS`, `navGroupLinks` | components.jsx | The site's navigation table — the grouped list of every reader-facing route, shared by the masthead dropdowns, the mobile menu, and (as a reference) the site index at `/explore`. Nothing in it may be date-derived or catalog-computed. Until September 2026 that was enforced by the shell generator, which baked the site masthead into index.html; the shell now bakes the homepage's own `HomeMasthead`, so the rule is policy without a guard. |
