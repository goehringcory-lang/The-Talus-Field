...
| `searchCatalog(query, { fuzzy, limit })` | page-search.jsx | The site's own matcher over the catalog and the standing-page table, exposed so other surfaces (the 404 page's "Did you mean") ask it rather than reimplementing the weights. `fuzzy` swaps a word that matches nothing for its nearest neighbour in the title vocabulary (two edits at five letters and up). Loaded with the `search` route bundle; callers outside it go through `ensureRoute("search")` first. |
...
| `Header` (returns `HomeMasthead` on the `home` route since September 2026, so `BottomNav` and the site masthead do not render on `/`), `HomeMasthead`, `HomeLink` (the homepage's one anchor component: native modified clicks, `#` hrefs scroll and focus a section, other hrefs go through `go()`, every click tracked), `Footer`, `BottomNav`, `BackToTop`, `Breadcrumbs`, `KeepGoing`, `ArticleCard`, `NewsletterInline` (with an optional `inputLabel` that renders a visible `<label>` bound to the input), `ExitIntentNewsletter`, `Placeholder`, `ResponsiveImage`, `MapLightbox`, `ShareRow`, `AffiliateNote`, `AvailabilityLink`, `LodgingCta`, `ExpediaBanner`, `expediaSearchUrl`, `EntranceWaits`, `WebcamStrip`, `GuidePromo`, `MotifMountains`, `MotifSun`, `MotifTrees` | components.jsx | Shared components (implicit globals). |
| `NAV_GROUPS`, `navGroupLinks` | components.jsx | The site's navigation table — the grouped list of every reader-facing route, shared by the masthead dropdowns, the mobile menu, and (as a reference) the site index at `/explore`. Nothing in it may be date-derived or catalog-computed. Until September 2026 that was enforced for the whole table by the shell generator, which baked the site masthead into index.html; the shell now bakes the homepage's own `HomeMasthead`, which renders the `plan` and `read` groups as its two menu panels, so the guard covers those two groups and the rule is policy only for the rest. |
...
| `readHistory` | components.jsx | Read-history store over safeStorage: `last`/`setLast`/`clearLast` (the unfinished article; the home resume band that read it was retired in the September 2026 homepage redesign, so page-article.jsx is now its only reader) and `done`/`markDone` (finished slugs, used to rank the article related rail unread-first). |
...
Four more touchpoints that are easy to miss and that nothing fails the build over: give it a `KEEP_GOING` entry in components.jsx (a route absent from that table renders no onward-links rail at all), put it in exactly one of `NAV_GROUPS` or `NAV_SECONDARY`, add it to the `Footer` column lists (hand-maintained, not derived from the nav tables), and hand-add it to the `## Reference pages` section of `llms.txt`, which `gen-seo-artifacts.mjs` does not regenerate. If the change touches `HomeMasthead`, `HomeHero`, or the Plan a Trip or Explore Yosemite group of `NAV_GROUPS`, run `npm --prefix scripts run home-shell` as well, since all of those bake into index.html (the homepage masthead renders those two groups as its menu panels). Finish with the shared `?v=` bump and `npm --prefix scripts run assets:stamp`.
...
| `guide_cta_click` | components.jsx (Footer "Field Guide" link, masthead nav), page-home.jsx (the Field Guide section's `/guide` button, a `HomeLink`, `location: home_content` with `target: /guide`), the shared `HpGuideBand` design-system band (a `HomeLink` to `/guide`, with the caller's `location`: `planning_hub` on `/planning` and `conditions` on `/conditions`, the same locations their `GuidePromo` used before the September 2026 design rollout), the shared `GuidePromo` component (fires with the caller's `location`; the static routes that mount it plus, since August 2026, page-article.jsx's end-of-article band on trails/planning/seasonal articles, `location: article_end`). Retired locations (annotate in GA4, do not reuse): `home_hero`, `home_band` (August 2026 homepage redesign, which made the guide one ask instead of two); `home_rail` (September 2026 visitor-first redesign, which retired the rail) |
...
| `affiliate_click` | app.jsx (delegated document listener on `a[data-aff-network]`). `aff_list` names the placement: `article_cta` (boxed LodgingCta in a body), `article_inline` (inline prose link in a body, any network), `article_town` (the per-town links in gateway-towns-compared), `article_picker` (the gateway-towns decision aid's recommendation link), `trip_selector` (the /planning plan output), `page_planning` / `page_itineraries` / `page_firefall` / `page_tioga` / `page_half_dome` / `page_distances` / `page_start_here` / `page_international` / `page_dates` (standing-page CTAs), `stay_search` (the hero search panel, the only link that carries dates; slug is the chosen place) / `stay_picker` (the road-first picker's answer, slug is the pick) / `stay_corridor` (the road-level search in each corridor's side column) / `stay_gateway` / `stay_property` (the named-stay rows under each corridor, per-property slug) / `stay_season` (the "when to stay where" blocks, per-season slug) / `stay_in_park_fallback` / `stay_camping_fallback` / `stay_closing` (the closing ask) / `stay_banner` (/stay), `map_sidebar`, the kit checklist's per-list slugs (page-kit.jsx), and `page` (the AvailabilityLink default). The legacy value `article` (all in-body placements, undifferentiated) ended 2026-08-01; a dashboard split by `aff_list` sees the old value stop and the three `article_*` values begin that day. `page_home` (the homepage rail's lodging unit) ended with the September 2026 visitor-first redesign, which carries no lodging link; do not reuse it. |
...
| `plate_zoom` | page-article.jsx (a photo plate opened in the lightbox; `slug`) |
...
| `cta_click` | components.jsx (masthead: `location: masthead_search` for search; `bottom_nav` with `target` for the mobile bottom tabs), Footer (`footer_index` for the site-index link); app.jsx (`notfound_suggest` with `target` for the 404 page's "Did you mean" links); components.jsx `HomeLink` on the homepage, always with `target` = the href (`home_navigation` for the homepage masthead's links and its "Get the app" button, and for every link inside its two menu panels, which `HomeMasthead` fires with `target` = the link's path, `home_hero` for the hero's two section buttons, `home_content` for every link in the utility strip, Start Here, the Field Guide section and the journal grid; a `HomeLink` to `/guide` fires `guide_cta_click` instead). The Sunday Letter unit reports through the newsletter events with `location: home_newsletter`. On the pages rebuilt on the design system (`DESIGN_ROUTES` in components.jsx) the same masthead fires `site_navigation`; `/planning` adds `planning_hero` (the head's two buttons), `planning_index` (the five-part index, fired by hand with `target: #part-N`), `planning_list` (a card in the filtered or full list) and `planning_part` (a card inside one of the five parts); `/conditions` adds `conditions_hero` (the head's two section buttons) and `conditions` (the section-heading links, shared with its guide band). Retired locations (annotate in GA4, do not reuse): `masthead_cta`, `masthead_now` (nav simplification pass); `home_door`, `home_month`, `home_path`, `home_strip_now` (August 2026 homepage redesign, which retired the audience links, the month planner, the Go Deeper row and the About strip); `home_index`, `home_dispatch`, `home_start_here` (September 2026 visitor-first redesign, which retired the index, the Bulletin band and the answers row) |
...
| `tfg.read.resume` | nothing, since September 2026 | One-shot handoff flag that the homepage resume band used to set; page-article.jsx still consumes it and jumps to the saved depth if it is present, but no surface writes it now. |
