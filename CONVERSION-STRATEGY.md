...
Phase 2 of the same pass added the date-awareness layer:

- **"When are you going?" month planner** under the utility row: twelve chips persisting `tfg.trip.month`, each opening a typical-season line (hedged like the PWA's `typical` confidence label; the Bulletin stays the source for current state) plus two matched evergreen reads, an Itineraries link, and a Bulletin link when the selected month is the current calendar month. Events: `trip_month_select{month}` and `cta_click{location: home_month, target}`.
- **Interest tagging begins:** with a month selected, the hero capture submits a second Buttondown tag `trip-<month>` alongside `home` — the first intent tag of the R3 segmentation backbone. At guide-launch time this yields a date-anchored segment (everyone whose trip month is inside the next ~90 days) without waiting on the R8 Worker subscribe endpoint.
